...
div_table_its <- cbind(metadata[, c('Gully_id', 'Group')], alpha_div, Faith = pd$PD, Distance = mod1$distances, Evenness = alpha_div$Shannon/log(alpha_div$Observed)) %>%
...
library(gghalves)
div_its_plot <- div_table_its %>% select(-c("Gully_id", "Observed", "se.chao1", "Simpson", "Faith", "Shannon")) %>%
...
colnames(phylaS1)<-colnames(phyla_tab_scale)[-c(1,2,18)]
data.frame(phylaS1)
```
### effect size plot
```{r, fig.align='center', fig.width=4.5, fig.height=6.5}
