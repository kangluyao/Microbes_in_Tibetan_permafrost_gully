...
```{r}
#Read in data
div_table <- read.table(file = "E:/thermokarst_gully/data/multifun/multifun_div_table_all.csv",
             sep = ',',  header = T, row.names = 1, stringsAsFactors = F)
...
multi_ave_func_df <- cbind(div_table, getStdAndMeanFunctions(multi_func_df, allVars, standardizeZScore))
```
...
```{r, fig.align='center', fig.width=12, fig.height=9.5}
#plot it
...
```{r, fig.align='center', fig.width=6.5, fig.height=8.5}
...
lmm.mat.cal <- function(y, x, method){
  ...
  df$Fun <- factor(df$Fun, ordered = T, levels = colnames(x))
  return(df)
}

diver_table <- multi_ave_func_df[ ,c("Plant_richness", "Soil_multidiv", "Ric_16s", "Actinobacteria",	"Alphaproteobacteria",	"Acidobacteria",	"Firmicutes",	"Betaproteobacteria", "Planctomycetes",	"Gammaproteobacteria",	"Bacteroidetes",	"Deltaproteobacteria", "Ric_its", "Ascomycota",	"Basidiomycota",	"Mortierellomycota", "Ric_pro", "Cercozoa",	"Conosa",	"Ciliophora",	"Chlorophyta",	"Apicomplexa",	"Ochrophyta",	"Pseudofungi",	"Lobosa",	"Sagenista",	"Opalozoa",	"Ric_anim", "Nematoda",	"Rotifera",	"Arthropoda",	"Annelida",	"Tardigrada", "Ric_kegg", "Ric_N", "Ric_P", "Ric_S")]
multifun_table <- multi_ave_func_df[ ,c("meanFunction", "WHC", "BG", "NAG", "LAP", "PPO", "H2O2", "SOC", "NO3_N", "NH4_N", "AP",  "AGB", "plant_pathogen_control", "Soil_respiration")]


lmm.matrix <- lmm.mat.cal(diver_table, multifun_table)
# lmm.matrix$Correlation[lmm.matrix$AdjPvalue >= 0.05] <- 0

#plot
p <- ggplot(aes(x = Fun, y = Diversity, fill = Correlation), data = lmm.matrix)+
  geom_tile() + scale_fill_gradient2(low="#2C7BB6", mid="white", high="#D7191C")+
  geom_text(aes(label=Significance), color="black", size = 5.5)+
  labs(y = 'Diversity index', x = 'Multifunctionality', fill= 'Correlation') +
...
richness_16s_phylum <- estimate_fun(phylo_16s)
richness_its_phylum <- estimate_fun(phylo_its)
richness_pro_phylum <- estimate_fun(phylo_protist)
richness_anim_phylum <- estimate_fun(phylo_animal)
```
