...
process_within_group <- null.df %>%
  ...
  ggplot(aes(process, rel_impor * 100)) +
  ...
  facet_grid(Taxa ~ Group, scales = 'free') +
    stat_compare_means(comparisons = my_comparisons_process, method = "wilcox.test", p.adjust.method = "BH") +
  labs(x = NULL, y = NULL) +
...
null_plot <- null.df %>%
  mutate(Group = factor(Group, levels = c("Control", "Collapsed"))) %>%
  ggplot(aes(Group, rel_impor, fill = process)) +
  geom_half_violin(position = position_nudge(x = 0.25), side = "r", width = 0.8, color = NA) +
  geom_boxplot(width = 0.4, size = 0.75, outlier.color = NA) +
  ...
  labs(x = NULL, y = NULL) +
  scale_fill_manual(values = c("#79ceb8", "#e95f5c", "#5cc3e8", "#ffdb00", "#1F77B4", "#AEC7E8", "#FF7F0E", "#FFBB78")) +
  facet_wrap(process ~ Taxa, scales = "free_y", ncol = 4) +
  main_theme
