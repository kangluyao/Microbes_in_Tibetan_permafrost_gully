...
Set work directory
```{r}
setwd('e:/thermokarst_gully/data')
...
```{r}
CO2_df <- flux %>% filter(Flux_type == "CO2") %>%
  select(c(Time, Gully_id, Rep, Uncollapsed, Collapsed)) %>%
  pivot_longer(cols = c("Uncollapsed", "Collapsed"), names_to = "Group", values_to = "Flux_value")
...
CH<sub>4</sub>:
```{r}
CH4_df <- flux %>% filter(Flux_type == "CH4") %>%
  select(c(Time, Gully_id, Rep, Uncollapsed, Collapsed)) %>%
  pivot_longer(cols = c("Uncollapsed", "Collapsed"), names_to = "Group", values_to = "Flux_value")
...
N<sub>2</sub>O:
```{r}
N2O_df <- flux %>% filter(Flux_type == "N2O") %>%
  select(c(Time, Gully_id, Rep, Uncollapsed, Collapsed)) %>%
  pivot_longer(cols = c("Uncollapsed", "Collapsed"), names_to = "Group", values_to = "Flux_value")
...
```{r}
flux_ave_df <- flux %>% select(c("Flux_type", "Gully_id", "Rep", "Uncollapsed", "Collapsed")) %>%
  group_by(Flux_type, Gully_id, Rep) %>%
  dplyr::summarise(across(, mean, na.rm = TRUE))
```

Test the difference in flux between Uncollapsed and collapsed using linear mixed model. First is CO<sub>2</sub>:
```{r}
library(lme4)
library(car)
...
my_comparisons <- list(c('Uncollapsed', 'Collapsed'))
flux_names <- list(
...
flux_plot <- flux_ave_df %>%
  select(c("Flux_type", "Gully_id", "Rep", "Uncollapsed", "Collapsed")) %>%
  pivot_longer(cols = c("Uncollapsed", "Collapsed"), names_to = "Group", values_to = "Flux_value") %>% 
...
fun_index <- c("Plant_shannon",	"Plant_richness",	"AGB",	"BGB",	"BG",	"NAG",	"LAP",	"PPO",	"H2O2",	
...
diff_in_env_plot <- env_S1 %>%
  ...
  annotate("rect", xmin = 11.5, xmax = 14.5, ymin = -2, ymax = 2, alpha = 0.2, fill = "#035096") +
  annotate("rect", xmin = 14.5, xmax = 15.5, ymin = -2, ymax = 2, alpha = 0.2, fill = "#00bfff") +
  main_theme +
