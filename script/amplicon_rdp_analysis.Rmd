...
```{r}
fm2 <- lmer(Evenness ~ Group + (1 | Gully_id), data = div_table)
summary(fm2)
car::Anova(fm2, type = 2)
```

```{r}
fm3 <- lmer(Shannon ~ Group + (1 | Gully_id), data = div_table)
summary(fm3)
car::Anova(fm3, type = 2)
```

```{r}
fm4 <- lmer(Faith ~ Group + (1 | Gully_id), data = div_table)
summary(fm4)
car::Anova(fm4, type = 2)
```

```{r}
fm5 <- lmer(Distance ~ Group + (1 | Gully_id), data = div_table)
summary(fm5)
...
library(gghalves)
div_plot <- div_table %>% select(-c("Gully_id", "Observed", "se.chao1", "Simpson", "Faith", "Shannon")) %>%
...
ggsave(file.path(save.dir, "./figs/diversity/diversity_plot.pdf"),
       div_plot, width = 4.5, height = 2, units = "in")
div_plot
```
...
#        beta_tax_plot, width = 7.6, height = 2.5, units = "in")
beta_tax_plot
```
...
#读取分组，合并数据并简单可视化
AVD_plot <- data.frame(Group = metadata$Group, AVD = avd) %>%
  mutate(Group = factor(Group, levels = c("Control", "Collapsed"))) %>%
...
diff_comp_plot <- avg_abun_in_control_plot %>% insert_right(eff_siz_tax_plot, width = 2)
ggsave(file.path(save.dir, "./figs/composition/diff_comp_plot.pdf"),
