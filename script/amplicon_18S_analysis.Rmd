...
div_table_18s <- cbind(metadata[, c("Sample_name", 'Gully_id', 'Group')], alpha_div, Faith = pd$PD, Distance = mod1$distances, Evenness = alpha_div$Shannon/log(alpha_div$Observed)) %>%
  mutate(Group = factor(Group, levels = c('Control', 'Collapsed'))) %>%
  mutate(Gully_id = factor(Gully_id, levels = c('EB', 'ML', 'RS', 'SLH', 'HSX', 'HH')))
# write.table(div_table_16s, file.path(save.dir, 'tables/div_table_16s.csv'),
...
library(lme4)
library(car)
fm1 <- lmer(Chao1 ~ Group + (1 | Gully_id), data = div_table_16s)
summary(fm1)
car::Anova(fm1, type = 2)
```

```{r}
fm2 <- lmer(Evenness ~ Group + (1 | Gully_id), data = div_table_16s)
summary(fm2)
car::Anova(fm2, type = 2)
```

```{r}
fm3 <- lmer(Shannon ~ Group + (1 | Gully_id), data = div_table_16s)
summary(fm3)
car::Anova(fm3, type = 2)
```

```{r}
fm4 <- lmer(Faith ~ Group + (1 | Gully_id), data = div_table_16s)
summary(fm4)
car::Anova(fm4, type = 2)
```

```{r}
fm5 <- lmer(Distance ~ Group + (1 | Gully_id), data = div_table_16s)
summary(fm5)
...
div_16s_gully_plot <- div_table_16s %>% 
...
library(gghalves)
div_16s_plot <- div_table_16s %>% select(-c("Sample_name", "Gully_id", "Observed", "se.chao1", "Simpson", "Faith", "Shannon")) %>%
...
#读取分组，合并数据并简单可视化
AVD_16s_plot <- data.frame(Group = metadata$Group, AVD = avd) %>%
  mutate(Group = factor(Group, levels = c("Control", "Collapsed"))) %>%
