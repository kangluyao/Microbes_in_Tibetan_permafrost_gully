...
```{r}
target_cat <- c("Thermophilic specific", "Amino acid utilization", "Ethanol fermentation", "Fatty acid degradation", "Aromatics degradation", "Complex carbon degradation", "Fermentation", "C1 metabolism", "Carbon fixation", "Methane metabolism", "Nitrogen cycling", "Sulfur cycling", "Hydrogenases", "As cycling", "Selenate reduction", "Iron cycling", "Manganese cycling")
...
pheatmap(dat2_ordered, 
         cluster_rows = F,
         cluster_cols = F,
         gaps_row = c(8, 11, 16, 21, 24, 30, 32, 33, 38, 40, 43, 44, 47, 55, 56),
         # gaps_col = 6,
...
```{r}
ctrl_input_table <- file.path(wd_fun, "metabolic/control/METABOLIC_Figures_Input/Metabolic_Sankey_diagram_input.txt")
collap_input_table <- file.path(wd_fun, "metabolic/collapsed/METABOLIC_Figures_Input/Metabolic_Sankey_diagram_input.txt")
```
...
table_ctrl <- read.table(ctrl_input_table, header = F, sep = "\t")
colnames(table_ctrl) <- c("Taxa", "Reaction", "Freq")
table_collap <- read.table(collap_input_table, header = F, sep = "\t")
colnames(table_collap) <- c("Taxa", "Reaction", "Freq")
```
...
```{r}
# reformate the data
table_all <- rbind(cbind(Group = rep("Control", nrow(table_ctrl)), table_ctrl),
...
nodes <- data.frame(
  name = c("Control", "Collapsed", taxa_names,
