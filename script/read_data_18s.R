...
wd_18s <- file.path(getwd(),"data/18S")
if (!dir.exists(wd_18s)) {
  dir.create(wd_18s)
}
...
## metadata
metadata <- read.delim(file.path(wd_18s, "./metadata.txt"), header = T, sep = "\t")
rownames(metadata) <- (metadata$Sample_id)

## tree
# tree <- read_tree(file.path(wd_18s, "./otus.nwk"))
## otu table
otu <- read.delim(file.path(wd_18s, "./otutab.txt"), header = T, row.names = 1, sep = "\t")
otu <- otu[, metadata$Sample_id[metadata$Sample_id %in% colnames(otu)]]
## rarefy out table
otu_rare <- read.delim(file.path(wd_18s, "./otutab_rare.txt"), header = T, row.names = 1, sep = "\t")
otu_rare <- otu_rare[, metadata$Sample_id[metadata$Sample_id %in% colnames(otu_rare)]]
## tax
tax <- read.delim(file.path(wd_18s, "./taxonomy.txt"), header = T, row.names = 1, sep = "\t")
tax <- as.matrix(tax)
# phyloseq object
