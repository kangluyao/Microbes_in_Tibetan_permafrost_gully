...
Composition
```{r}
# Extrac the taxa with the relative abundance higher than 1%
subextract_fun <- function(phylo, taxa_level, perc) {
  subphylo <- tax_glom(phylo, taxa_level, NArm = F)
  subphylo.rel  = transform_sample_counts(subphylo, function(x) x * 100 / sum(x))
...
otu_rel_abun_1perc_phyla_16s <- subextract_fun(phylo_16s, "Phylum", 1)
# extract the taxa at class level
otu_rel_abun_1perc_class_16s <- subextract_fun(phylo_16s, "Class", 1)
...
otu_rel_abun_1perc_its <- subextract_fun(phylo_its, "Phylum", 1) %>%
  filter(!Phylum %in% c("Unassigned"))

# extrac the Protistan phyla with the relative abundance higher than 1%
otu_rel_abun_1perc_pro <- subextract_fun(phylo_protist, "Phylum", 1) %>%
  filter(!Phylum %in% c("Unassigned"))

# extrac the Animal phyla with the relative abundance higher than 1%
otu_rel_abun_1perc_anim <- subextract_fun(phylo_animal, "Class", 1) %>%
...
library(cowplot)
venn_plots <- plot_grid(p1, p2, p3, p4, ncol = 2, align = "v")
venn_plots
```
...
```{r, fig.align='center', fig.width=2.5, fig.height=7.5}
#16S
...
pcoa_fun_plot <- data.frame(Group = metadata$Group, scores(ord.fun)) %>%
  ...
  theme(legend.position = "none",
        ...
        panel.background = element_blank(), 
        panel.border = element_rect(fill = NA, colour = "black"))
pcoa_tax <- cowplot::plot_grid(pcoa_16s_plot, pcoa_its_plot, pcoa_pro_plot, pcoa_anim_plot, ncol = 1)
ggsave(file.path(save.dir, "./figs/composition/pcoa_tax.pdf"),
       pcoa_tax, width = 1.8, height = 5, units = "in")
pcoa_tax
```
...
similar_data <- data.frame(Group = similar_16s_data$Group,
                            ...
                            distance_anim = similar_anim_data$distance)
similar_data
```
...
spec_gen_pro <- spec.gen(phylo_protist@otu_table %>% t(), 
                         niche.width.method = "levins", perm.method = "quasiswap",
                         n = 1000, probs = c(0.05, 0.95))
# write.table(spec_gen_pro, file.path(save.dir, "/tables/spe_gen/spec_pro_95.csv"), sep = ",")
spec_gen_animal <- spec.gen(phylo_animal@otu_table %>% t(), 
                            niche.width.method = "levins", perm.method = "quasiswap",
                            n = 1000, probs = c(0.05, 0.95))
...
vars <- c('G1_C', 'G1_T', 'G2_C', 'G2_T', 'G3_C', 'G3_T', 'G4_C', 'G4_T', 'G5_C', 'G5_T', 'G6_C', 'G6_T')
# Assuming vars is defined somewhere earlier in your code
similar_spec_protist_data <- lapply(vars, function(x) usedist::dist_subset(tax_spec_pro_dist, grep(x, metadata$Sample_name, value = TRUE))) %>%
  do.call(cbind, .) %>%
  data.frame() %>%
  gather("tem_group", "distance") %>%
  cbind(Gully_id = rep(c('EB', 'ML', 'RS', 'SLH', 'HSX', 'HH'), each = 20),
...
similar_gen_protist_data <- lapply(vars, function(x) usedist::dist_subset(tax_gen_pro_dist, grep(x, metadata$Sample_name, value = TRUE))) %>%
  do.call(cbind, .) %>%
  data.frame() %>%
  gather("tem_group", "distance") %>%
  cbind(Gully_id = rep(c('EB', 'ML', 'RS', 'SLH', 'HSX', 'HH'), each = 20),
...
similar_protist_data <- data.frame(Group = similar_spec_protist_data$Group,
                               ...
                               distance_pro_spec = similar_spec_protist_data$distance,
                               distance_pro_gen = similar_gen_protist_data$distance)
...
distance_index <- c("distance_16s", "distance_16s_gen", "distance_16s_spec", 
                    "distance_its", "distance_its_gen", "distance_its_spec", 
                    "distance_pro", "distance_pro_gen", "distance_pro_spec", 
                    "distance_anim", "distance_anim_gen", "distance_anim_spec")
...
#plot
rep_str = c("distance_16s" = "Overall species",
            "distance_16s_gen" = "Generalists",
            "distance_16s_spec" = "Specialists",
            "distance_its" = "Overall species",
            "distance_its_gen" = "Generalists",
            "distance_its_spec" = "Specialists",
            "distance_pro" = "Overall species",
            "distance_pro_gen" = "Generalists",
            "distance_pro_spec" = "Specialists",
            "distance_anim" = "Overall species",
            "distance_anim_gen" = "Generalists",
            "distance_anim_spec" = "Specialists"
)
...
extract_fun <- function(df) {
  simi_group_df <- dist_groups(1-df, gp) # transform the distance matrices into similarity matrices
...
compo_venn_eff.size_resis_plot <- cowplot::plot_grid(single_dist_comparison, resistance_comparison, ncol = 2, align = "h")
ggsave(file.path(save.dir, './figs/composition/spe_gen_comparison.pdf'), compo_venn_eff.size_resis_plot, width = 3.5, height = 5)
compo_venn_eff.size_resis_plot
```


Explore the difference in abundance of genus
```{r}
...
class_1perc_16s <- subextract_fun(phylo_16s, "Class", 1) %>% 
  filter(Class != "Unassigned") %>% #remove the Unassigned rows
  rename("Taxa" = "Class")
class_1perc_its <- subextract_fun(phylo_its, "Class", 1) %>% 
  filter(Class != "Unassigned") %>% #remove the Unassigned rows
  rename("Taxa" = "Class")
class_1perc_pro <- subextract_fun(phylo_protist, "Class", 1) %>% 
  filter(Class != "Unassigned") %>% #remove the Unassigned rows
  rename("Taxa" = "Class")
order_1perc_anim <- subextract_fun(phylo_animal, "Order", 1) %>% 
  filter(Order != "Unassigned") %>% #remove the Unassigned rows
  rename("Taxa" = "Order")
#Combine all taxa table
all_1perc_taxa <- rbind(class_1perc_16s, class_1perc_its, class_1perc_pro, order_1perc_anim)

# Pivot the dataframe to reshape it
abun_df <- all_1perc_taxa %>%
  pivot_longer(cols = -Taxa, names_to = "Sample_id", values_to = "Value") %>%
...
```{r}
taxa_name <- all_1perc_taxa$Taxa
tax_abun_stats <- abun_df %>% group_by(Group) %>%
  get_summary_stats(taxa_name, type = "common") %>% #or using type = "mean_sd"
  mutate(Group = factor(Group, levels = c("Uncollapsed", "Collapsed"))) %>%
  arrange(variable, Group)
tax_abun_stats
```

## Linear mixed models test the effect of permafrost thawing on relative abundance of each taxa
```{r}
# determine the effect size of the permafrost thawing for the diversity indexes
abun_scale <- abun_df %>% 
  select(c("Group", "Gully_id", c(taxa_name))) %>%
  mutate(across(where(is.numeric), scale)) %>%
  as.data.frame() %>%
  mutate(Group = factor(Group, levels = c("Uncollapsed", "Collapsed")))
...
# codes for calculating the effect size refer to wu et al. 2022:https://github.com/Linwei-Wu/warming_soil_biodiversity.
abun_S1 <- sapply(3:ncol(abun_scale), function(j) {
    message("Now j=", j, " in ", ncol(abun_scale), ". ", date())
    if (length(unique(abun_scale[, j])) < 3) {
        result <- rep(NA, 50)
    } else {
        fm1 <- lmer(abun_scale[, j] ~ Group + (1 | Gully_id), data = abun_scale)
...
all_tax_abun_comparison <- abun_S1 %>% 
  ...
  tibble::rownames_to_column(., "variables") %>%
  mutate(sig = as.vector(unlist(lapply(Group.P, p.stars)))) %>%
  mutate(variables = factor(variables, levels = rev(taxa_name))) %>%
  mutate(colour = case_when(GroupCollapsed.mean <= 0 & Group.P <= 0.05 ~ "Negative",
                            GroupCollapsed.mean > 0 & Group.P <= 0.05 ~ "Positvie",
                            Group.P > 0.05 ~ "Neutral")) %>%
  ggplot(aes(x = variables, y = GroupCollapsed.mean, colour = colour)) +
  ...
  annotate("rect", xmin = 0.5, xmax = 7.5, ymin = -2, ymax = 2, alpha = 0.1, fill = "#ffdb00") +
  annotate("rect", xmin = 7.5, xmax = 21.5, ymin = -2, ymax = 2, alpha = 0.1, fill = "#f19837") +
  annotate("rect", xmin = 21.5, xmax = 29.5, ymin = -2, ymax = 2, alpha = 0.1, fill = "#e56eee") +
  annotate("rect", xmin = 29.5, xmax = 41.5, ymin = -2, ymax = 2, alpha = 0.1, fill = "#5fb236") +
  main_theme +
  theme(legend.position = "none",
...
#   dir.create(file.path(save.dir, "figs/env/"))
# }
# ggsave(file.path(save.dir, "./figs/composition/all_tax_abun_comparison.pdf"),
#        all_tax_abun_comparison, width = 2.5, height = 5, units = "in")
all_tax_abun_comparison
```


Test the relationship between resistance and effects of permafrost collapse among all taxa at the phylum level. Firstly, we calculate the effect size of permafrost collapse on the community comnposition of each phylum
```{r}
tax_phyla_16s.ids <- c("Actinobacteria", "Acidobacteria", "Firmicutes", "Planctomycetes", "Bacteroidetes")
tax_class_16s.ids <- c("Alphaproteobacteria", "Betaproteobacteria", "Gammaproteobacteria", "Deltaproteobacteria")

df <- NULL
x <- tax_phyla_16s.ids
for (i in 1:length(x)) {
    message("Now i = ", x[i], ". ", date())
    # subset the phyloseq objects for each phyla
    ps <- subset_taxa(phylo_16s, Phylum == x[i])
  
    # extract the OTU table from the ps
    otu.tab <- as.data.frame(ps@otu_table)
  
    # determine the dissimilarity matrix based on the bray-curties distance
    dist_matrix <- vegdist(t(otu.tab), "bray")
    
    # set a vector containing sample name strings for extracting part of dist matrixes
    vars <- c('G1_C', 'G1_T', 'G2_C', 'G2_T', 'G3_C', 'G3_T', 'G4_C', 'G4_T', 'G5_C', 'G5_T', 'G6_C', 'G6_T')
  
    # extract the within-site values and transform the dist matrix into three columns
    temp_df <- lapply(vars, function(x) usedist::dist_subset(dist_matrix, grep(x, metadata$Sample_name, value = TRUE))) %>%
      do.call(cbind, .) %>% as.data.frame() %>%
      gather("tem_group", "values") %>%
      select(-tem_group)
    colnames(temp_df) = x[i]
    if (is.null(df)) {
      df = temp_df
      } else {
        df = cbind(df, temp_df)
...
df_16s_phyla = cbind(Gully_id = rep(c('EB', 'ML', 'RS', 'SLH', 'HSX', 'HH'), each = 20),
           Group = rep(c('Uncollapsed', 'Collapsed'), each = 10, times = 6), df) %>%
  mutate(Gully_id = factor(Gully_id, levels = c('EB', 'ML', 'RS', 'SLH', 'HSX', 'HH')),
         Group = factor(Group, levels = c('Uncollapsed', 'Collapsed')))
...
for (i in 1:length(x)) {
    message("Now i = ", x[i], ". ", date())
    # subset the phyloseq objects for each phyla
    ps <- subset_taxa(phylo_16s, Class == x[i])
...
df_16s_each_taxa = cbind(df_16s_phyla, df)


#Fungi
its_ids <- c("Ascomycota", "Basidiomycota", "Mortierellomycota")
...
for (i in 1:length(x)) {
    message("Now i = ", x[i], ". ", date())
    # subset the phyloseq objects for each phyla
    ps <- subset_taxa(phylo_its, Phylum == x[i])
...
pro_ids <- c("Cercozoa", "Conosa", "Ciliophora", "Chlorophyta", "Apicomplexa", 
             "Ochrophyta", "Pseudofungi", "Lobosa", "Sagenista", "Opalozoa")
...
for (i in 1:length(x)) {
    message("Now i = ", x[i], ". ", date())
    # subset the phyloseq objects for each phyla
    ps <- subset_taxa(phylo_protist, Phylum == x[i])
...
anim_ids <- c("Nematoda", "Rotifera", "Arthropoda", "Annelida", "Tardigrada")
...
for (i in 1:length(x)) {
    message("Now i = ", x[i], ". ", date())
    # subset the phyloseq objects for each phyla
    ps <- subset_taxa(phylo_animal, Class == x[i])
...
df_dist_all <- data.frame(df_16s_each_taxa, df_its_phyla, df_pro_phyla, df_anim_phyla)

#Using linear mixed model to determine the effect size of permafrost collapse
library(lme4)
library(lmerTest)
tax_name <- unique(tax_name)
# determine the effect size of the permafrost thawing for the diversity indexes
dist_phyla_scale <- df_dist_all %>% 
  select(c("Group", "Gully_id", tax_name)) %>%
  mutate(across(where(is.numeric), scale)) %>%
  mutate(Group = factor(Group, levels = c("Uncollapsed", "Collapsed"))) %>%
  select(where(~ !any(is.na(.))))

# codes for calculating the effect size refer to wu et al. 2022:https://github.com/Linwei-Wu/warming_soil_biodiversity.
dist_phyla_S1 <- sapply(3:ncol(dist_phyla_scale), function(j) {
  message("Now j=", j, " in ", ncol(dist_phyla_scale), ". ", date())
  if (length(unique(dist_phyla_scale[, j])) < 3) {
    result <- rep(NA, 10)
  } else {
    fm1 <- lmer(dist_phyla_scale[, j] ~ Group + (1 | Gully_id), data = dist_phyla_scale)
...
colnames(dist_phyla_S1)<-colnames(dist_phyla_scale)[-c(1:2)]
effect_size_each_taxa <- dist_phyla_S1 %>% t() %>% as.data.frame() %>% select("GroupCollapsed.mean", "GroupCollapsed.se", "Group.P") %>% rownames_to_column("Taxa")
effect_size_each_taxa
```

Secondly, we determine the resistance of each phylum
```{r}
gp <- paste(metadata$Gully_id, metadata$Group, sep = "_")
extract_fun <- function(df) {
  simi_group_df <- dist_groups(1-df, gp) # transform the distance matrices into similarity matrices
...
for (i in 1:length(x)) {
    ...
    # determine the dissimilarity matrix based on the bray-curties distance
    temp_list <- list(vegdist(t(otu.tab), "bray"))
    names(temp_list) <- x[i]
    if(is.null(list)) {
      list = temp_list
    } else {
      list= c(list, temp_list)
    }
}
list_phyla_16s_dist <- list
...
list_each_taxa_16s_dist <- c(list, list_phyla_16s_dist)
...
for (i in 1:length(x)) {
    message("Now i = ", x[i], ". ", date())
    # subset the phyloseq objects for each phyla
    ps <- subset_taxa(phylo_animal, Class == x[i]) #here should be Class level
...
list_phyla_anim_dist <- list
#Combine all lists for all taxa
list_dist_all_taxa <- c(list_each_taxa_16s_dist, list_phyla_its_dist, list_phyla_pro_dist, list_phyla_anim_dist)
#Determine all resistance of all taxa
simi_between_phyla_df <- lapply(list_dist_all_taxa, extract_fun)
#Reformate the resistance data
resitance_each.taxa_df <- do.call("rbind", simi_between_phyla_df) %>%
  mutate(Taxa = rep(names(simi_between_phyla_df), each = 6))
resistance_each_taxa_stats <- resitance_each.taxa_df %>% group_by(Taxa) %>%
  get_summary_stats(Resistance, type = "common") #or using type = "mean_sd"
resistance_each_taxa_stats
```

```{r}
plot_df <- resistance_each_taxa_stats %>% select(Taxa, mean, se) %>% 
  left_join(effect_size_each_taxa, by = "Taxa") %>% 
  mutate(Taxa = factor(Taxa, levels = tax_name)) %>% 
  mutate(Group = c(rep("Bacteria", 9),
                   rep("Fungi", 3),
                   rep("Protist", 10),
                   rep("Animal", 5)))

resitance_eff.size_plot <- plot_df %>% mutate(GroupCollapsed.mean = abs(GroupCollapsed.mean)) %>%
  ggplot(aes(mean, GroupCollapsed.mean))+
  #errorbar for x variable
  geom_errorbar(aes(xmin = mean - se, 
                    xmax = mean + se), linewidth = 0.4, width = 0)+
  #errorbar for y variable
  geom_errorbar(aes(ymin = GroupCollapsed.mean - GroupCollapsed.se,
                    ymax = GroupCollapsed.mean + GroupCollapsed.se), linewidth = 0.4, width = 0)+
  #point
  geom_point(aes(color = Taxa), size = 2)+
  #regression line
  ...
  guides(color=guide_legend(override.aes = list(size=5,alpha=1)),
         size = "none")+
  #axis labes
  labs(x = "Resistance", y = "Effect size of permafrost collapse", color = NULL) +
  main_theme + theme(legend.position = "none")
resitance_eff.size_plot
```
