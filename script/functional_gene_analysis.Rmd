...
```
Combine the plot
```{r, fig.align='center', fig.width=8.9, fig.height=5}
library(cowplot)
left_col <- plot_grid(pcoa_fun_plot, volca_plot_edger, labels = c('A', 'B'), label_size = 6, ncol = 1, align = "v")
fig2 <- plot_grid(left_col, kegg_heatmap, labels = c('', 'C'), label_size = 6,rel_widths = c(1, 2.2), ncol = 2)
# fig2 <- ggdraw() +
#   draw_plot(pcoa_fun_plot, x = 0, y = 0.5, width = 0.3, height = 0.5) +
#   draw_plot(volca_plot_edger, x = 0, y = 0, width = 0.3, height = 0.5) +
#   draw_plot(kegg_heatmap, x = 0.3, y = 0, width = 0.7, height = 1) +
#   draw_plot_label(label = c("A", "B", "C"), size = 6,
#                   x = c(0, 0, 0.3), y = c(1, 0.5, 1))
ggsave(file.path(save.dir, "/figs/Fig2.pdf"),
       fig2, width = 8.9, height = 5, units = "in")
fig2
```
